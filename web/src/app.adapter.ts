...
export const AppAdapter = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      let accessToken: string = await asyncStorageHelpers.get(
        StorageKey.ACCESS_TOKEN
      );
      let refreshToken: string = await asyncStorageHelpers.get(
      ...
      if (code) {
        ...
        accessToken = await asyncStorageHelpers.get(
            StorageKey.ACCESS_TOKEN
        );
        refreshToken = await asyncStorageHelpers.get(
            StorageKey.REFRESH_TOKEN
        );
        //   return;
        // }
